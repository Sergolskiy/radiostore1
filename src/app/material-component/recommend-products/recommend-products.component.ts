...
import {RecommendProductsService} from "../../services/recommend-products.service";
...
@Component({
  ...
  styleUrls: ['./recommend-products.component.scss']
})
export class RecommendProductsComponent implements OnInit{
    ...
    constructor(private service: RecommendProductsService, private orderService: OrderService, private router: Router, public dialog: MatDialog) { }

    ngOnInit() {
        this.service.getRecommendProducts().subscribe((res:any) => {
            if(res.products) {
    ...
    openDialogForDelete(slug): void {
        ...
        dialogRef.afterClosed().subscribe(result => {
            ...
            if(this.checkDelete == false) {
                this.service.deleteRecommendProduct(slug).subscribe((res:any) => {
                    if(res.status) {
    ...
    setProduct(form: NgForm) {
        ...
        this.service.setRecommendProduct(form.value).subscribe((res:any) => {
            if(res.product) {
